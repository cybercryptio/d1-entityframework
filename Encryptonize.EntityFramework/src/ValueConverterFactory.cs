...
internal static class ValueConverterFactory
{
    ...
    internal static ValueConverter<string, string> CreateStringConverter(IEncryptonizeClient client)
    {
        return new ValueConverter<string, string>(v => StringEncryptor(v, client), v => StringDecryptor(v, client));
    }

    internal static ValueConverter<byte[], byte[]> CreateBinaryConverter(IEncryptonizeClient client)
    {
